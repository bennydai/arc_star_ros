...
int main(int argc, char **argv) {
  ...
  nh_private.getParam("sensorWidth", sensorWidth);
  nh_private.getParam("sensorHeight", sensorHeight);

  if (sensorWidth != detector.getSensorWidth() ||
    sensorHeight != detector.getSensorHeight())
  {
    ROS_INFO("Sensor width and height does not match");
    ROS_INFO("Launch file parameters: %i, %i", sensorWidth, sensorHeight);
